        ...
        combinePaperDelivery: function(deliveryList, typeName) {
            ...
            if (combinePaperDeliveryFeature) {
                combinedList = _.filter(deliveryList, function(id) {
                    id = parseInt(id);
                    if (typeName === 'deliveryType') {
                        if (_.contains(idList, id) === true) {
                            id = primaryPaperId;
                        }
                    }
            ...
            // combine paper Delivery Type: replace 6,7 with 5
            if (typeName === 'deliveryType') {
                for (var index = 0; index < combinedList.length; index++) {

                    if (_.contains(idList, parseInt(combinedList[index])) === true) {
                        combinedList[index] = primaryPaperId;
                    }
                }
            }
            combinedList = _.uniq(combinedList);
