...
define([
    ...
    var EventHelper = (function() {
        ...
        function scrollEventHeader(element, scrollUp) {
            var containers = $('.gh-container, #content_container');
        ...
        var isRecoExperience = function() {
            // TODO: cleanup ticket reco global variable to be consistent with experiences
            //       creating globals.experience.ticketReco
            //       in the future we should move all experience variants to globals.experience.*
            return globals.TicketReco.showTicketReco !== globals.TicketReco.experience.DEFAULT;
        };
        ...
        var constructMboxData = function(eventModel) {
            ...
            mboxEventObject.event = {
                ...
                ticketReco: globals.TicketReco.hasTicketReco
            };
        ...
        var mBoxSuccess = function() {
            var testKeyValues = {
                ...
                'quantitySelectorStyle_11_2016': abNameSpace.quantitySelectorStyle_11_2016,
                'mapTakeover': abNameSpace.mapTakeover,
                'mapTakeoverPosition': abNameSpace.mapTakeoverPosition,
                'mapZoneSelection': abNameSpace.mapZoneSelection,
                'DefaultSort': abNameSpace.DefaultSort
        ...
        var isVisaCheckoutOrderDetailEnabled = getFeatureFn('eventpage.visaCheckoutOrderDetail', 'boolean');
        ...
        var isMapZoneSelected = function() {
            return getFeatureFn('eventpage.mapZoneSelection', 'boolean')();
        };
        ...
        var isEnabledQuantitySelectorStyle_11_2016 = function() {
            var urlParameterValue = getUrlQuery('qtyddab'),
                mboxValue = SH.mbox && SH.mbox.quantitySelectorStyle_11_2016,
                featureToggleValue = getFeatureFn('event.quantitySelectorStyle_11_2016')(),
                state = FALSE;


            if (urlParameterValue && urlParameterValue === 'true') {
                state = TRUE;
            } else if (urlParameterValue && urlParameterValue === 'false') {
                state = FALSE;
            } else if (typeof(mboxValue) !== 'undefined') {
                state = mboxValue;
            } else if (featureToggleValue) {
                state = featureToggleValue;
            }

            return state;
        };

        var getQuantitySelectionConfig = function() {

            var dropDownClass = isEnabledQuantitySelectorStyle_11_2016()? 'evt-dropdown-select-quantity-filter': '',
            ...
            if (this.isTablet() || this.isMobile()) {
                quantitySelectorStyle.cssClass += ' evt-dropdown';
                quantitySelectorStyle.filterType = 'select';
            }

            return quantitySelectorStyle;
        };

        var cleanEventPageCookies = function() {
            var cookies = document.cookie.split(';'),
                thisEventCookie = globals.cookiePrefix,
                thisCookie,
                domainMap = urlParser.getDomainMap(),
                domainName = '.' + domainMap.domainName + '.' + domainMap.domainSuffix;

            for (var i = 0; i < cookies.length; i++) {
                thisCookie = cookies[i];
                if (thisCookie.indexOf('event_') >= 0 && thisCookie.indexOf(thisEventCookie) === -1) {
                    Foobunny.storage.expireCookie(thisCookie.substring(0, thisCookie.indexOf('=')), domainName);
                }
            }

        };

        var isMapTakeOverEnabled = function() {
            var mapTakeover = getUrlQuery('mapto'),
                mboxValue = SH.mbox && SH.mbox.mapTakeover,
                featureToggle = getFeatureFn('event.mapTakeover')(),
                state = FALSE,
                mobile = isMobile();
            ...
            if (mapTakeover && mapTakeover === 'true') {
                state = TRUE;
            } else if (mapTakeover && mapTakeover === 'false'){
                state = FALSE;
            } else if (typeof(mboxValue) !== 'undefined') {
                state = mboxValue;
        ...
        return {
            ...
            isDisplayWithFeesToggle: getFeatureFn('eventpage.showWithFees', 'boolean'),
            displaySellerContactInfo: getFeatureFn('eventpage.displaySellerContactInfo', 'boolean'),
            displayTicketlistFooter: getFeatureFn('eventpage.displayTicketlistFooter', 'boolean'),
            isDisplayTotalCost: getFeatureFn('eventpage.displayTotalCost', 'boolean'),
            ...
            getQuantitySelectionConfig: getQuantitySelectionConfig,
            isEnabledQuantitySelectorStyle_11_2016: isEnabledQuantitySelectorStyle_11_2016,
            isMapZoneSelected: isMapZoneSelected,
            cleanEventPageCookies: cleanEventPageCookies,
            isRecoExperience: isRecoExperience,
            isVisaCheckoutOrderDetailEnabled: isVisaCheckoutOrderDetailEnabled,
            isMapTakeOverEnabled: isMapTakeOverEnabled
        };
    }());
    return EventHelper;
});
