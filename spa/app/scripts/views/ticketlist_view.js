...
define([
    ...
    var TicketlistView = Foobunny.BaseView.extend({
        ...
        uiEl: {
            ...
            '$ticketListContainer': '#ticketlist_container'
        },
        ...
        events: {
            'scroll #ticketlist_container' : 'highlightOrLoadMore',
        ...
        initialTicketFetch: function() {
            ...
            if ($('#single-ticket').is(':visible')) {
                ...
                this.uiEl.$ticketListContainer.removeClass('hide');
            } else {
        ...
        highlightOrLoadMore: function(evt) {
            ...
            if (!$eventHeader.hasClass('scrolled-up') && window.innerWidth < globals.screen_breakpoints.tablet && !$('#header #hamburger').hasClass('clicked')) {
                ...
                $('.gh-container').one('webkitAnimationEnd mozAnimationEnd animationend', function(e) {
        ...
        highlightSectionOnMap: function(sectionId, price, isZoneTicket) {
            if (!this.seatmap || this.isMapStatic) {
                return;
            }
        ...
        seeAllTickets: function() {
            ...
            //also hide vfs
            this.publishEvent('enlargevfs:hide');
            $('#ticketlist_container').removeClass('hide');
        ...
        */
        scrollToView: function($ticket) {
            var $ticketList = this.$el.parent(),
                ticketListBoundingBox = $ticketList[0].getBoundingClientRect(),
                ticketBoundingBox = $ticket[0].getBoundingClientRect(),
                tlHeight = $ticketList.height(),
                tktHeight = ticketBoundingBox.bottom - ticketBoundingBox.top,
                tktTop = $ticket.parent().position().top,
                alignTicket = '';

            if (ticketBoundingBox.top < ticketListBoundingBox.top ||
                tktHeight > tlHeight) {
                alignTicket = 'top';
            } else if (tktHeight + ticketBoundingBox.top > ticketListBoundingBox.bottom) {
                alignTicket = 'bottom';
            }

            if (alignTicket !== '') {
                $ticketList.scrollTop(tktTop);
            }
        ...
        afterRender: function() {
            ...
            $("#ticketlist_container").scroll(this.highlightOrLoadMore.bind(this));
        }
    });
