define([
    ...
    var SingleListingModel = Foobunny.BaseModel.extend({
        ...
        updateTotalCost: function() {
            var price = globals.PDO.withFees ? this.get('buyerSeesPerProduct') : this.get('pricePerProduct');
            this.set({
                usePrice: price,
                totalCost: (price.amount * this.get('qty')).toFixed(2)
            });
        },
        ...
        parse: function(result) {
            ...
            if (!listing.seatNumberLast && listing.seatNumbersArray.length > 1) {
            ...
            if (listing.deliveryMethods && listing.deliveryMethods.length > 0) {
                ...
                listing.deliveryTypes = listing.deliveryMethods.map(function(deliveryMethod) {
                    var name = DeliveryHelper.getDeliveryMethodName(deliveryMethod.id, deliveryMethod.name);
                ...
                _.each(listing.deliveryTypes, function(deliveryType) {
                    var delivery = DeliveryHelper.combinePaperDelivery(deliveryType || [], 'deliveryType');
                    if (delivery.length) {
                        listing.deliveryTypeList = [{
                            id: delivery[0],
                            deliveryAttribute: delivery[1]
                        }];
                    } else {
                        listing.deliveryTypeList = [];
                    }
