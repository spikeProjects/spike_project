define(['app/scripts/views/ticketlist_view',
        'models/listing_model',
        'global_context',
        'globals',
        'foobunny',
        'models/inventory_metadata_model',
        'collections/inventory_collection'],
    function(TicketlistView, ListingMode, gc, globals, Foobunny, InventoryMetadataModel, InventoryCollection) {
    describe('Ticket List View', function() {
        'use strict';

        var view, model, inventoryResponse;
        ...
        it('should define TicketlistView type as function', function() {
            expect(typeof TicketlistView).toEqual('function');
        });

        it('should define TicketlistView', function() {
            //var eventDeferred = Foobunny.utils.deferred();
            //view = new TicketlistView();
            // view = new TicketlistView({
                // 'fetchOnInitialize': false,
                // 'eventDeferred' : Foobunny.utils.deferred(),
                // 'eventPromise' : eventDeferred.promise(),
                // 'inventoryFetchPromise' : null,
                // 'subModels' : {
                    // 'metadata' : new InventoryMetadataModel()
                // },
                // 'lastchanged' : 'initial',
                // 'collection' : new InventoryCollection(gc.event_id),
                // 'subCollections' : {
                ...
                // 'deliveryMappings' : [],
                // 'attributeMappings' : [],
                // 'attributeMappingsForIcons' : [],
                // 'deliveryTypeList' : [],
                // 'listingAttributeList' : [],
                // 'isMapStatic' : false,
                // 'isMapAvailable' : false,
                // 'seatmap' : null,
                // 'mapHighlightedSectionId' : null,
                // 'mapHighlightedSectionPrice' : '',
                // 'enablePrimaryPartner' : false,
                // 'isHost' : false,
                // 'viewingAllTickets' : (gc.ticket_id ? false : true),
                // 'totalListings' : 0,
                // 'isLoading' : false
            // });

            //model = new ListingMode(inventoryResponse);

            //console.log('inside the beforeEach ticketlistView model: ', model);
            //console.log('inside the beforeEach ticketlistView view: ', view);
            //console.log('inside the ListingMode: ', mode);
            //console.log('inside the ticketlistView: ', view);
